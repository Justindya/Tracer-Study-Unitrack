    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl p-8 shadow-sm border border-gray-100 flex flex-col md:flex-row items-start gap-8 mb-6">
            ...
            <div class="w-24 h-24 bg-blue-600 rounded-2xl flex flex-col items-center justify-center text-white flex-shrink-0 shadow-lg shadow-blue-200">
                <span class="text-4xl font-bold leading-none">{{ $event->tanggal->format('d') }}</span>
                <span class="text-sm font-bold uppercase mt-1">{{ $event->tanggal->format('M') }}</span>
            </div>
            
            <div class="flex-1">
                <h1 class="text-3xl font-bold text-gray-900 mb-3">{{ $event->judul }}</h1>
                <div class="flex flex-wrap items-center gap-6 text-sm text-gray-600">
                    <span class="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                        <i class="fas fa-map-marker-alt text-red-500"></i> {{ $event->tempat }}
                    </span>
                    <span class="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                        <i class="fas fa-clock text-yellow-500"></i> {{ $event->jam }} WIB
                    </span>
                </div>
            </div>
