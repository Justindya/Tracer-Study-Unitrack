                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-blue-50/50 rounded-lg border border-blue-100">
                            <div>
                                ...
                                <p class="text-sm font-bold text-blue-700 mt-1 capitalize">{{ str_replace('_', ' ', $tracer->status) }}</p>
                            </div>
                            <div class="md:col-span-2">
                                <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Tanggal Mulai</p>
                                <p class="text-sm font-semibold text-gray-800 mt-1">{{ $tracer->tanggal_mulai->format('d M Y') }}</p>
                            </div>
                        </div>
                        ...
                        @if ($jawaban && isset($soalLabels[$status]))
                                    ...
                                    @foreach ($soalLabels[$status] as $i => $label)
                                        <li class="flex flex-col sm:flex-row p-4 hover:bg-gray-50 transition">
                                            ...
                                            <div class="w-full sm:w-1/2 mt-1 sm:mt-0">
                                                <span class="text-sm font-semibold text-gray-900">{{ $jawaban->{'soal_' . ($i + 1)} ?? '-' }}</span>
                                            </div>
                                        </li>
                                    @endforeach
                                </ul>
                        ...
                        <div class="mt-8 pt-5 border-t border-gray-100 flex gap-3">
                            <a href="{{ route('user.tracer.edit', $tracer->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-6 py-2.5 rounded-lg font-semibold text-sm transition shadow-sm flex items-center gap-2">
