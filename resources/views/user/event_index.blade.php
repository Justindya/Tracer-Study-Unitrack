    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg transition duration-300 flex flex-col md:flex-row items-center gap-6 group h-full">
                    ...
                    <div class="bg-blue-50 text-blue-600 rounded-xl w-24 h-24 flex flex-col items-center justify-center flex-shrink-0 border border-blue-100 group-hover:bg-blue-600 group-hover:text-white transition duration-300 shadow-sm">
                        <span class="text-3xl font-bold block leading-none">{{ $event->tanggal->format('d') }}</span>
                        <span class="text-xs font-bold uppercase mt-1">{{ $event->tanggal->format('M') }}</span>
                    </div>

                    <div class="flex-1 text-left w-full">
                        <h3 class="text-lg font-bold text-gray-900 mb-2 group-hover:text-blue-600 transition">{{ $event->judul }}</h3>
                        
                        <div class="space-y-1 text-sm text-gray-500 font-medium">
                            <p class="flex items-center gap-2">
                                <i class="fas fa-map-marker-alt text-gray-400 w-4"></i> {{ $event->tempat }}
                            </p>
                            <p class="flex items-center gap-2">
                                <i class="fas fa-clock text-gray-400 w-4"></i> {{ $event->jam }} WIB
                            </p>
                        </div>
                    </div>

                    <div class="w-full md:w-auto mt-4 md:mt-0">
                        <a href="{{ route('user.events.show', $event->id) }}" class="block w-full md:w-auto bg-white border border-blue-600 text-blue-600 hover:bg-blue-50 px-6 py-2 rounded-lg font-bold text-sm text-center transition shadow-sm">
                            Lihat Detail
                        </a>
                    </div>
        ...
        @else
            <div class="col-span-full text-center py-16 bg-white rounded-3xl border border-dashed border-gray-300">
                ...
                <a href="{{ route('user.events.index') }}" class="text-blue-600 font-bold text-sm hover:underline">Reset Filter</a>
            </div>
        @endif
