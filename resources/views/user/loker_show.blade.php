    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex flex-col md:flex-row items-start gap-6 mb-6">
            ...
            <div class="flex gap-3 mt-4 md:mt-0 w-full md:w-auto">
                ...
                @else
                    <a id="btn-apply" href="mailto:{{ $loker->kontak }}?subject=Lamaran: {{ $loker->judul }}" class="flex-1 md:flex-none bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg font-bold transition shadow-sm shadow-blue-200 flex items-center justify-center gap-2 text-sm">
...
{{-- MODAL KONFIRMASI (UI UX 2026: Absolute Center, Dark Backdrop Blur) --}}
<div id="apply-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-gray-900/60 backdrop-blur-sm transition-opacity opacity-0 duration-300">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center transform scale-95 transition-transform duration-300 mx-4" id="modal-box">
        ...
        <p class="text-sm text-gray-500 mb-6 leading-relaxed">
            Apakah Anda telah mengirimkan email lamaran ke <span class="font-bold text-gray-800">{{ $loker->perusahaan }}</span>?
        </p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if(btnApply){
            btnApply.addEventListener('click', function() {
                isApplying = true;
                sessionStorage.setItem('pending_application_id', '{{ $loker->id }}');
            });
        }
        ...
        document.addEventListener('visibilitychange', function() {
            ...
            if (document.visibilityState === 'visible' && isApplying && pendingId == '{{ $loker->id }}') {
        ...
        btnConfirm.addEventListener('click', function() {
            ...
            fetch('{{ route("user.lokers.store") }}', {
                ...
                body: JSON.stringify({ loker_id: '{{ $loker->id }}', status: 'terkirim' })
            })
            .then(res => res.json())
            .then(data => {
                isApplying = false;
        ...
        // Bookmark logic
        const jobData = { id: "{{ $loker->id }}", title: "{{ $loker->judul }}", company: "{{ $loker->perusahaan }}", url: "{{ route('user.lokers.show', $loker) }}" };
